...
- `ILLUNA_API_URL`: full HTTPS workflow endpoint configured in the server `.env` and displayed only in the signed-in API access example. No endpoint is committed to source; the personal request is hidden until this value is set. Public demo and documentation examples always use `https://api.example.com/v1/adapt` and `YOUR_API_KEY`. Examples use `x-api-key`, `message`, `ui_elements` and the current nested `updates` response. No requests are sent by these pages.
