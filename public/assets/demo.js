(() => {
      ...
      demoShell.addEventListener("click", event => {
        if (!(event.target instanceof Element) || !event.target.closest("[data-next-step]")) return;
        demoState.step = (demoState.step + 1) % taskIds.length;
        renderDemo("Showing task " + (demoState.step + 1) + " of 3: " + words.en.names[demoState.step] + ".");
        demoShell.querySelector("[data-next-step]").focus({preventScroll: true});
      });
      ...
      openEditor(new URLSearchParams(window.location.search).get("view"));
      renderDemo("", false);
})();
